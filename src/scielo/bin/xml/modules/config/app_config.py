...
class AppConfiguration(object):
    ...
    def __init__(self, filename, web_config_filename=None):
        self._data = {}
        c = 'utf-8'
        ...
        web_config = []
        if web_config_filename is not None:
            web_config = fs_utils.read_file_lines(web_config_filename, c)
        for item in fs_utils.read_file_lines(filename, c) + web_config:
            if '=' in item:
    ...
    def is_web_access_enabled(self):
        return self._data.get('WEB_ACCESS_ENABLED')

    @property
    def web_access_proxy_server_and_port(self):
        return self._data.get('WEB_ACCESS_PROXY')
