...
def is_fulldate(label, dateiso):
    ...
    msg = []
    if not y > 0:
        msg.append('{}: {}. '.format(label, dateiso) + _('{value} is an invalid value for {label}. ').format(value=y, label='year (' + label + ')'))
    if not 0 < m <= 12:
        msg.append('{}: {}. '.format(label, dateiso) + _('{value} is an invalid value for {label}. ').format(value=m, label='month (' + label + ')'))
    if not 0 < d <= 31:
        msg.append('{}: {}. '.format(label, dateiso) + _('{value} is an invalid value for {label}. ').format(value=d, label='day (' + label + ')'))
    if len(msg) == 0:
        try:
            r = datetime(y, m, d)
        except:
            msg.append('{}: {}. '.format(label, dateiso) + _('{value} is an invalid value for {label}. ').format(value=d, label=label + ': day '))
    return msg
